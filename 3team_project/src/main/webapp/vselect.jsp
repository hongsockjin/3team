...
<%
	...
	List<LookDto> list = dao.getVmList();
...
<body>
	...
	<div class="machine">
		...
		<div class="m1">
			<table border="1">
				...
				<col width="10%">
				<tr>
					<th>브랜드</th><th>제품명</th><th>가격</th><th>흡입량</th><th>무게</th><th>배터리(h)</th><th>에너지효율</th><th></th>
				</tr>
				<%
				...
				<tr id="abc">
					...
					<td><%=dto.getVname()%></td>
					<td><%=dto.getVprice()%></td>
					<td><%=dto.getVsuction()%></td>
					<td><%=dto.getVkg()%></td>
					<td><%=dto.getVtime()%></td>
					<td><%=dto.getVdegree()%></td>
					<td><button  class="bbb" value="[브랜드:<%=dto.getSeq()%> | 제품명:<%=dto.getVname()%> | 가격:<%=dto.getVprice()%> | 무게:<%=dto.getVsuction()%> | 에너지효율:<%=dto.getVdegree()%>]" onclick="Choose1(this.value)" >선택</button></td>
				</tr>
				<%
				}
				%>
		...
		<div class="m2">
			<table border="1">
				...
				<tr>
					...
					<td><%=dto.getVtime()%></td>
					<td><%=dto.getVdegree()%></td>
					<td><button  class="ccc" value="[브랜드:<%=dto.getSeq()%> | 제품명:<%=dto.getVname()%> | 가격:<%=dto.getVprice()%> | 무게:<%=dto.getVsuction()%> | 에너지효율:<%=dto.getVdegree()%>]" onclick="Choose2(this.value)">선택</button></td>
				</tr>
				<%
				}
				%>
	...
	<div class="compare">
		...
		<button class="comb" onclick="Math()">계산해보기</button>
		<div class="com2">
...
<script type="text/javascript">
 	function Choose1(a) {
 		var das = "<tr><td id='a'>"+a+"</td></tr>";
 		$(".pqpq1").append(das);
 		
 	}
 	function Choose2(b) {
 		var das = "<tr><td id='b'>"+b+"</td></tr>";
 		$(".pqpq2").append(das);
 	}
 	function Math() {
		var i = $("#a").attr(vname);
